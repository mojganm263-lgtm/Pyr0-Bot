...
def setup(bot: commands.Bot, SessionLocal):
    """Register all commands to the bot tree."""
    tree = bot.tree

    setup_channels(tree, SessionLocal)
    setup_scores(tree, SessionLocal)
    setup_import_export(tree, SessionLocal)
    setup_leaderboards(tree, SessionLocal)
    setup_history(tree, SessionLocal)
...
# ---------- Scoring commands ----------
def setup_scores(tree, SessionLocal):
...
def setup_history(tree, SessionLocal):
    ...
    async def diff(interaction: Interaction, name: str, category: str):
        ...
        try:
            ...
            prev = score.history[-1].new_value

            await interaction.response.send_message(
                f"Score differences for {name} in {category}:\n" + "\n".join(differences)
            )
        finally:
            session.close()
